# Note: If this works with resizing and moving the tkinter window I will be just as shocked as you are
...
# Setting bounds for 3 vertices of triangle. v2 bounds are listed as an addition to the x-coordinate of v1 (v1 and v2 always share y coordinates), and v3 is listed as an addition in the -y direction to the y value of v1 and v2
v1_bounds = [[-400, 400], [1, center_y - 200]]
v2_adder_bounds = [100, 200]
v3_adder_bounds = [100, 200]

# Choosing bounds at random and applying to get initial points
...
# Create a small point that will follow the cursor
...
def update_cursor_position():
    ...
    # Find unit vector of cursor with respect to origiin
    ...
    # Calculate the end points of the line to cap its distance at 100px
...
# Main loop to do HC calculations
while True:
    hit = False

    # Collect cursor points adjusted to drawn-in coordinate frame
    cursor_point_x, cursor_point_y = update_cursor_position()

    # Need to transform triangle vertices from graphical window coordinate frame to drawn-in coordinate frame
    ...
    # Find HC lines needed for hitting calculations
    l_cursor = np.cross(origin, p)
    l12 = np.cross(v1, v2)
    l23 = np.cross(v2, v3)
    l31 = np.cross(v3, v1)

    # Find HC intersection points
    intersection12 = np.cross(l_cursor, l12)
    intersection23 = np.cross(l_cursor, l23)
    intersection31 = np.cross(l_cursor, l31)

    # Checking if intersections are ideal points and skipping if they are (as ideal points represent parallel lines and those don't intersect)
    # If points not parallel, checking if intersection point lies within the bounds of the two vertices of the traingle's edge
